...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET["id"])) {
        ...
        if (!$row) {
            header("location:/CRM/indexvend.php");
            exit;
        } else {
            $nom = $row["nom"];
            $prenom = $row["prenom"];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $email = $_POST["email"];
    $wilaya = $_POST["wilaya"];
    if (empty($nom) || empty($prenom) || empty($telephone) || empty($email) || empty($wilaya) ) {
        $errorMessage = "Veuillez remplir tous les champs.";
    } else {
        // Mettre à jour les informations du client dans la base de données
        $sql = "UPDATE vendeur SET nom='$nom', prenom='$prenom', telephone='$telephone', email='$email',wilaya='$wilaya' WHERE id=$id";
        if ($connection->query($sql) === TRUE) {
            $successMessage = "Vendeur mis à jour avec succès.";
        } else {
            $errorMessage = "Erreur lors de la mise à jour du vendeur: " . $connection->error;
        }
    }
}
...
<body class="bg-gray-100">
    <div class="container mx-auto my-5">
        <h2 class="text-3xl font-bold mb-8 text-center">Modifier vendeur</h2>
        ...
        <form method="post" class="max-w-md mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="wilaya" name="wilaya" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Entrez votre email" value="<?php echo isset($wilaya) ? $wilaya : ''; ?>">
            </div>
            <div class="flex items-center justify-between">
                ...
                <a href="indexvend.php" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Retour à la page d'accueil</a>
            </div>
        </form>
    </div>
</body>
