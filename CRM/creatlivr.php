...
$nom = $prenom = $telephone = $email = $bureau =$password = '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nom) || empty($prenom) || empty($telephone) || empty($email)|| empty($password) || empty($bureau)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO livreur (nom, prenom, telephone, bureau, email, password, date_creation) VALUES (:nom, :prenom, :telephone, :bureau, :email, :password, NOW())");
...
<body class="bg-gray-100">
    <div class="container mx-auto my-5">
        <h2 class="text-3xl font-bold mb-8 text-center">Nouveau livreur</h2>
        ...
        <form method="post" class="max-w-md mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="indexlivr.php" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Retour au page d'acceuil</a>
            </div>
        </form>
    </div>
</body>
