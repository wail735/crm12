...
$nom = $prenom = $telephone = $email = $bureau = $wilaya =$password = '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nom) || empty($prenom) || empty($telephone) || empty($email)|| empty($password) || empty($wilaya) || empty($bureau)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO vendeur (nom, prenom, telephone, wilaya, bureau, email, password, date_creation) VALUES (:nom, :prenom, :telephone, :wilaya, :bureau, :email, :password, NOW())");
...
<body class="bg-gray-100">
    <div class="container mx-auto my-5">
        <h2 class="text-3xl font-bold mb-8 text-center">Nouveau vendeur</h2>
        ...
        <form method="post" class="max-w-md mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="mb-4">
                <label for="wilaya" class="block text-gray-700 text-sm font-bold mb-2">Wilaya</label>
                <input type="text" id="wilaya" name="wilaya" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Entrez votre wilaya" value="<?php echo $wilaya; ?>">
            </div>
            <div class="mb-4">
                <label for="bureau" class="block text-gray-700 text-sm font-bold mb-2">Bureau</label>
                <input type="text" id="bureau" name="bureau" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Entrez votre bureau" value="<?php echo $bureau; ?>">
            </div>
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="indexvend.php" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Retour au page d'acceuil</a>
            </div>
        </form>
    </div>
</body>
